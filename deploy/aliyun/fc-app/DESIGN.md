...
>
> **Step 2 修向（CR ga7zzjob 终裁：(c) 显式钉变体先行，(a) glibc 备梯）**：**主修 (c)**（CR ga7zzjob 2 旋钮 → CR 9qtkhcse 4 消费方，Dev e13zdrmu 源码级证伪 2 旋钮的**覆盖范围**，**增量非翻案**——方向「绕过探测组件」反被坐实）= **5 个宣告式钉，一个都不能少**（5 个探测消费方各读各的 env，无法合并，是最小完整集；#3 经 02b fail 实证证伪 migrate 期 SE 解析有效性降为 download 期 belt〔load-bearing 为 build 段 #1〕留作不删，#5 补 migrate 期 SE 路径钉与 ④ 同机制族零 probe）：
>
...
> ③ Dockerfile.fc **runtime 段** `ENV PRISMA_CLI_BINARY_TARGETS=linux-musl-openssl-3.0.x` **重申**（**02b fail qrrhoenn 实证证伪 migrate 期 SE 解析有效性后降级**：`PRISMA_CLI_BINARY_TARGETS` 是 **generate/download 期变量**——仅 `@prisma/engines` postinstall/`binariesExist()` 下载面消费，**CLI bundle `prisma/build/index.js` grep -c=0 零引用**（Dev 5ws6rrho 源码钉），migrate 期 schema-engine resolver `wu()` 从不读它；02b③红实证 env baked 在场而 CLI 仍搜裸名 `schema-engine-linux-musl` 非钉品→#3 对 migrate 路 = **结构性 no-op**。**真正缺陷是注释谎报承重理由非变量在场**（CR kiyiys1u 裁，TL 96pwd1oz 委 CR 裁 #3）：runtime 段无 generate（generate 在 build 段），故原「generate 期一致性锚点」承重理由实证为空；诚实化为「download 期 belt，runtime 段无消费点，load-bearing 为 build 段 #1，migrate 期 SE 解析走 #5 不走此」。**变量保留不删**（TL 96pwd1oz 终裁不删 + 零行为影响留作 belt）；migrate 期 SE 真正路径钉见 ⑤）；
> ④ Dockerfile.fc **runtime 段** `ENV PRISMA_QUERY_ENGINE_LIBRARY=/app/node_modules/.prisma/client/libquery_engine-linux-musl-openssl-3.0.x.so.node`（Dev 缺口②，**04 生死线 non-negotiable**：`@prisma/client` loader `library.js` env 钉优先于运行时探测；不钉则 FC 无 openssl CLI→探测落 `linux-musl`（1.1.x 味）→加载 `native` 焙的坏 `.so.node`→04 必炸。loader `za()`=`{library: env}[type] ?? prismaPath`，env 置则**原样当加载路径返回，零 binaryTarget 名→路径解析**（Dev t6ekqku4 源码 + fq74qtf9 运行时 A/B 双锁：env=名→`Unable to require(名)` dlopen MODULE_NOT_FOUND loud 炸 / env=全路径→engine 加载成功死于不可达 DB P1001）⟹ **名值不解析，全路径=唯一正解**（CR 9qtkhcse「优先用名…名不行退全路径」被源码证伪，全路径=CR 自己 hedged 退路之正退，CR zqrpp96z 翻案记账）。失效模式：`Qn()` existsSync 校验**仅 ia32 分支**，**FC=x64**→错路径/名值=dlopen **loud** error（错误信息含该值，第一手可见）**非静默回退**；哑只哑在「误以为名值合法」，失效瞬间 loud。全路径落点三方咬合确定（workspaces hoist + Dockerfile L32 `COPY --from=build /app/node_modules` 整体带过 + Dev generate log `to ./../../node_modules/@prisma/client`）→ `/app/node_modules/.prisma/client/libquery_engine-linux-musl-openssl-3.0.x.so.node`；⓪ ls 探针仍留（belt + 钉落点），「防哑雷」紧迫性降级）。
> ⑤ Dockerfile.fc **runtime 段** `ENV PRISMA_SCHEMA_ENGINE_BINARY=/app/node_modules/@prisma/engines/schema-engine-linux-musl-openssl-3.0.x`（02b fail qrrhoenn 证伪 #3 migrate 期 SE framing 后的修正，**与 ④ 同机制族**：env-var MAP `{"query-engine":"PRISMA_QUERY_ENGINE_BINARY","libquery-engine":"PRISMA_QUERY_ENGINE_LIBRARY","schema-engine":"PRISMA_SCHEMA_ENGINE_BINARY"}`（CR kiyiys1u 沙箱独立复证）——schema-engine 与 #4 的 libquery-engine **同族平行**各有全路径 env 钉（故 #4 用 LIBRARY〔libquery-engine type〕#5 用 BINARY〔schema-engine type〕非不一致）。migrate 期 schema-engine resolver `wu(binaryName,from)`=fetch-engine `resolveBinary`（「Could not find schema-engine binary. Searched in:」即其 throw）解析序 ①显式 path 参数→②`df(e)`=getBinaryEnvVarPath：查 map 得 env 名 `PRISMA_SCHEMA_ENGINE_BINARY`，env 置则 `path.resolve(cwd,值)` **原样当加载路径返回零 probe 依赖**，existsSync 不过即 loud throw「Env var X is provided but provided path Y can't be resolved」；③此后才进 probe（get-platform）搜裸名 `schema-engine-<探测值>`。env 先于 probe——02b③红=无 #5 env→落③ probe default openssl-1.1.x→搜裸名 `schema-engine-linux-musl`→不在（只焙 openssl-3）→abort（deploy 报错逐字 = `wu()` 末尾 throw，CR kiyiys1u nail）；落 #5 env→直接命中已焙 openssl-3 品→migrate 零 probe 依赖。**三源实证咬合**（CR kiyiys1u formal ratify #5 不 (A)）：Deploy txcot6ms 部署镜像 7075f92 in-image grep `prisma/build/index.js` env-var MAP（非沙箱更权威）+ Dev 5ws6rrho 源码 `wu()` resolveBinary 解析序 + runtime A/B（env→不存在路径→loud throw 含该值 / env→真实 engine 全路径→`migrate diff` 全量 SQL exit 0=路径原样消费 pin 端到端生效）+ TL 96pwd1oz 终裁 #5 不 (a)。**落点三方咬合**：workspaces hoist + Dockerfile L32 `COPY --from=build /app/node_modules` 整体带过 + Deploy ⓪ls 实证在场 → `/app/node_modules/@prisma/engines/schema-engine-linux-musl-openssl-3.0.x`（**注意异于 ④ QE 的 `.prisma/client/`，两 engine 异目录**）；绝对路径避 `df` 的 cwd-resolve 歧义；失效 loud（x64-loud 族续立非静默）；execve 已证 +x（probe ② `schema-engine --version` OK，env 路径不走 chmodPlusX 但 baked 物 postinstall 期已 +x）。**端到端已证**（Deploy qaoahsw6 re-bake tag 2febd02 ③green：02b migrate GREEN，migration `20260721125210_init` applied to `mycoffee_prod`〔RDS pgm-1udy8leas753wge4 内网〕，零 `Could not find schema-engine binary`，零 `Downloading Prisma` 行——#5 路径钉坐实，SE resolver 读 env 路径直用已焙 openssl-3 品零 probe 零下载；CR jf6mady3 裁 ③ 已是最强端到端证，`prisma:fetch-engine:env` debug 行 belt 第二落点钉非必需不补跑）。**比 (a) apk add openssl 严格优**：纯宣告式路径钉同 ④ 机制族零 probe 组件 image 不增包，与 (c)「绕过不可靠探测组件」100% 一致；(a)=重回 probe（修好探测再信它恰恰是 (c) 当初要绕的）；
> **为何显式钉非修探测（apk add openssl 补探测输入）**：这次咬我们的就是探测组件——绕过不可靠组件不修好再信它，一个机制族（显式声明）不两个（声明+探测）。**(a) glibc 基镜（node:20-slim debian）= 备梯非弃**（CR 9qtkhcse + Dev e13zdrmu 修正理解）：实证缺陷=探测失输非 musl 固有，musl×openssl「持续风险源」是理论威胁；ldd 满足的 musl-openssl-3 品若 FC 仍 DLOADER FAIL=musl 固有证据首次成立，届时 glibc 的大 diff（基镜/体积/冷启拉取/entrypoint 全层重验）才挣得它的钱。**修正（关键）**：(a) 非纯换基镜——Dev glibc 沙箱同样焙了 `rhel-openssl-1.1.x` engine（探测默认在 glibc 上照样触发），故 (a)=换基镜**+装 openssl CLI**（探测阶段需 CLI 存在才能选对变体），否则同炸。这反强化 (c)：(c) 全程不依赖探测组件，(a) 还得信探测修好——备梯位照认，理解更新。**(b) 拒**（alpine 3.21+ 已撤 libssl1.1 包+双 libssl 并存=症状补丁打地鼠），**(d) 拒**（glibc 机跑 migrate 仅修 migrate，FC serving 仍炸，单独不够，deploy/TL 认同拒）。
>
> **预宣验证梯（证据先于 FC，不再赌）**：新 bake 后、03 前，Deploy read-only **4 探针**（CR 9qtkhcse 收 Dev ④，前 3 廉价/诊断/互补全留）：① `ldd .prisma/client/libquery_engine-linux-musl-openssl-3.0.x.so.node`=libssl.so.3/libcrypto.so.3 零 not found（廉价诊断，链接面）② `schema-engine-linux-musl-openssl-3.0.x --version` 直 exec 出版本（migrate 面）③ 02b（新 tag）migrate 绿（ECS 路证；**已 green qaoahsw6 tag 2febd02**——migration applied to mycoffee_prod + 零 `Could not find schema-engine binary` + 零 `Downloading Prisma` 行 = #5 SE 路径钉坐实：env→baked 全路径→零 probe→零下载，belt-and-suspenders）④ 镜像内 `PrismaClient.$connect` 探针（alpine runtime + 无 openssl CLI=FC 环境复刻）→ `QE OK`=运行时选择面先绿，03/04 只剩 FC 平台自身变量——**④ 是 04 生死面的决定证**（对 query engine 它包含①；判别器②「musl-inherent 首证」升梯框定不变）。→ 4 步全过=机制预期再跑 03+04。若 4 步全绿而 FC 仍 DLOADER FAIL→真沙箱 dlopen 限制证据首成（判别器② 原假设复活，升梯 (a)/静态引擎换推理框架）。**封签 fcd43385 变**（预期中 Step 2 条件触发）→ re-bake 新 tag 新 digest → CR 一轮重审记新封签。
>
